...
public:
	...
	Mjpeg(int port = 80, int q = 95, int nworkers = 16) : HTTPServer(port, nworkers), q(q) {
		...
		(*this)("", [](boost::asio::ip::tcp::iostream &net, std::string , std::string , std::string ) {
			...
			net << "<html style='height: 100%;'><body style='height: 100%; background:#000000 no-repeat center center url(/mjpeg/);background-size:contain;'></body></html>";
			net << "\r\n\r\n";
		});
	}
